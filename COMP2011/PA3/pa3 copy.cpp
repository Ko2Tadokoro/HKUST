...
bool ll_insert_prerequisite(Course* head, const char targetCode[MAX_CODE], const char preCode[MAX_CODE]) {//ok?
...
/*     if (strcmp(preCode, targetCode) > 0)
    ...
    {
        if (strcmp(preCode, course_ptr->code) != 0 || strcmp(targetCode, course_ptr->code) != 0)//Not the same code
            return false;
...
bool ll_insert_exclusion(Course* head, const char targetCode[MAX_CODE], const char excludeCode[MAX_CODE]) {//ok?
    ...
    for (Course* course_ptr = head; course_ptr != nullptr;)
    {
        if (strcmp(excludeCode, course_ptr->code) != 0 || strcmp(targetCode, course_ptr->code) != 0)//Not the same code
            return false;
    ...
    if (targetCourse->exclusions == nullptr)//empty prereq list
        {
...
bool ll_delete_prerequisite(Course* head, const char targetCode[MAX_CODE], const char preCode[MAX_CODE]) {//hard
    ...
    for (course_ptr = head; (course_ptr != nullptr) && (strcmp(course_ptr->code, targetCode) != 0);)
        course_ptr = course_ptr->next;

        if (strcmp(course_ptr->code, targetCode) == 0)//search for prerequisite now
        {

            CourseItem* tempitem_ptr = nullptr;
            //CourseItem* previous_ptr = nullptr;

            if (strcmp(course_ptr->prerequisites->course->code, preCode) == 0)//applicable to 1st node
            {
                tempitem_ptr = course_ptr->prerequisites->next;
                delete course_ptr->prerequisites;
                course_ptr->prerequisites = tempitem_ptr;

                return true;
            }
            else
            {
                for (CourseItem *previous_ptr = course_ptr->prerequisites, *item_ptr = previous_ptr->next; item_ptr != nullptr;)//here item_ptr != nullptr is fine
                {                                                                                                               //since no dereferencing

                    if (strcmp(item_ptr->course->code, preCode) == 0)//applicable to the range of 2st to last nodes
                    {
                        //ONLY prereq list points to item_ptr           
                        tempitem_ptr = item_ptr->next;

                        previous_ptr = tempitem_ptr;

                        delete item_ptr;
                        item_ptr = tempitem_ptr;
...
bool ll_delete_exclusion(Course* head, const char targetCode[MAX_CODE], const char excludeCode[MAX_CODE]) {//hard
    ...
    Course* targetCourse = nullptr, *excluCourse = nullptr;

    //targetCode DNE OR excludeCode DNE
    for (Course* course_ptr = head; course_ptr != nullptr;)
    {
        if (strcmp(excludeCode, course_ptr->code) != 0 || strcmp(targetCode, course_ptr->code) != 0)//Not the same code
            return false;
    ...
    //targetCode AND excludeCode EXIST

    //targetCode DOES NOT have excludeCode as a exclusion
    for (targetCourse=head; targetCourse != nullptr;)
    {
...
//The course code is at least 2 characters (including NULL) and at most 10 characters (including NULL). 
//You don't need to check if the input is invalid.

//The course title is at least 2 characters (including NULL) and at most 100 characters (including NULL). 
//You don't need to check if the input is invalid.
...
/*     for (Course* course_ptr = head; course_ptr != nullptr;)
    {
        if (strcmp(c,course_ptr->code) == 0)//same code, finding the target course
            {
                if (course_ptr == head)//first course
                    {
                        delete 
                    }
